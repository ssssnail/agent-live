...
| 文件名 / URL 参数 | Style | Layout | Agent Skin | NPC | Life | Atmosphere |
| --- | --- | --- | --- | --- | --- | --- |
| `demo-office` | `pixel-classic` | `demo-office` | `tiny-developers` | `none` | `none` | `default` |
| `lively-office` | `pixel-classic` | `lively-office` | `tiny-developers` | `office-staff` | `office-basics` | `default` |
| `night-shift` | `pixel-classic` | `lively-office` | `tiny-developers` | `office-staff` | `office-basics` | `rainy-night` |
| `cozy-studio` | `warm-studio` | `lively-office` | `studio-team` | `office-staff` | `office-basics` | `default` |
| `old-school-office` | `warm-studio` | `old-school-office` | `studio-team` | `old-school-staff` | `old-school-routines` | `default` |
| `boardroom-office` | `pixel-classic` | `boardroom-office` | `tiny-developers` | `boardroom-staff` | `boardroom-routines` | `default` |

这些组合说明了两种常用方式：

- 只换 Atmosphere：`lively-office` → `night-shift`。
...
| `boardroom-office` | 一间大会议室 | 1 个桌首席、6 个两侧席、1 个末端席 | 主屏 / 决策墙、AV 控制台、会议电话、资料车 |
...
Atmosphere 是对 Style 的局部覆盖，所以不需要复制完整 token。当前已经实现的环境效果只有 `rain-window`；其他新效果需要先在 renderer 中实现。
